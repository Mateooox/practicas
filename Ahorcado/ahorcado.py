...
def inputSecret():
    ...
    # Desarrolle el cuerpo de la función aquí... 
    pSec=input("Ingrese la palabra secreta\n")
    # Valor retornado
    return pSec
...
def loadWords(fileName):
    ...
    palabrasr=open(fileName,"r")
    palabras=palabrasr.read()
    palabrasr.close()    
	# Cadena retornada
    return palabras
...
def pickWord(palabras,separador):
    ...
    '''
    # Desarrolle el cuerpo de la función aquí...
    palabras_div=palabras.split(separador)
    conta=countWords(palabras, ",")
    a=random.randint(0,conta)
    palabra=palabras_div[a-1]
...
def obtenerLetrasDisponibles(letrasIntentadas):
    ...
    # Desarrolle el cuerpo de la función aquí...
    resto = ""#letras intentatadas
    letras = "qwertyuiopasdfghjklñzxcvbnm"

    for char in letras:
        if char in letrasIntentadas:
            resto += "_ "
...
def palabraAdivinada(palabra,letrasIntentadas):
    ...
    for char in palabra:  
        ...
        if char not in letrasIntentadas+" ":
            inc = False
            return inc
        else:
            inc = True
